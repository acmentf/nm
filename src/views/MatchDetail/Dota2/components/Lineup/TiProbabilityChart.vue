...
<template>
    <div :class="$options.name" alt="LOL胜率等图probability组件" v-bind:[$options.name]="$options.version">
        ...
        <div class="tip blue">{{data.team_name_home||'[主队]'}}：{{data.team_rate_home}} <span></span></div>
        <div class="tip red">{{data.team_name_away||'[客队]'}}：{{data.team_rate_away}} <span>Pick顺序→</span></div>
    </div>
</template>
...
export default {
  ...
  methods: {
      initChart(){
          var _params="?x-oss-process=image/resize,m_fill,h_140,w_140,limit_0";
          _params =""; //去掉裁剪
          var _tmp =[],_box =[];
          var data = this.data;
          ...
          for (var index = 0; index < data.role_list.length; index++) {
            ...
            var _data =  [index, element.rate * 1];
              var _item = {
                    value:_data,
                    symbol:'image://'+element.role_img_url +_params
                };
                _tmp.push(_item);
        ...
        //   console.log("_tmp_tmp_tmp",this.chartOption);
        ...
        chartOption:{
                        ...
                        */
                        ...
                        　　　　for (var i = 0; i < params.length; i++) {
                                var _css ='width:40px; text-align: right; white-space: nowrap;overflow: hidden;text-overflow: ellipsis;    vertical-align: bottom; display: inline-block; ';
                                var _tmp = "<b style='color:rgba(255, 255, 255, 0.7);font-size:10px;"+_css+"'>" + params[i].seriesName+"</b> "+params[i].marker+" <b style='color:"+params[i].color+";font-size:10px;display: inline-block;' >"+params[i].data +"</b><br />";
                               //   console.log(_tmp);
                        　　　　　　res += _tmp;
                        　　　　}
