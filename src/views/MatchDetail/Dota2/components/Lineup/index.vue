<!--
 * @Author: FeikeQ
 * @Date: 2020-11-18 17:10:00
 * @LastEditTime: 2020-12-09 13:56:12
 * @LastEditors: FeikeQ
 * @FilePath: /ti-wap/src/views/MatchDetail/Dota2/components/Lineup/index.vue
 * @Description: 
-->
<template>
  <div class="fixedContent" :class="$options.name" v-bind:[$options.name]="$options.version">
    ...
    >
      <van-tab v-for="(item, index) in tabList" :title="item.lable" :key="index">
        ...
        <div class="content">
          <div v-if="item">
            <TiBarLOL title="队伍数据对比" :data="item.lineup.rcffj_list" />
            <br/>
            <TiProbability title="胜率一血率等排名数据" :data="item.lineup.battle_info" v-model="tabIndex" />
            <TiRecord
              v-if="item.lineup.battle_info"
              :title="item.lineup.battle_info[tabIndex].page_top.page_name||'战绩比对'"
              :data="item.lineup.battle_info[tabIndex].page_down"
            />
          </div>
        </div>
        </van-pull-refresh>
